...
export default class Beyblade {
	constructor(name = "Generic", imageName, x, y, vx, vy, rotationDirection = 1, weight, type) {
		...
		this.rotation = 0;
		// Constants
		this.gravityForce = 0.008 / (this.weight / 100); //0.008
	}
	...
	updateRotation() {
		...
		if (this.rotationDirection === 1) this.rotation = (this.rotation + Math.min(((this.rpm / RPM) * 5).toFixed(2), 1)) % 360;
		else this.rotation = (this.rotation - Math.min(((this.rpm / RPM) * 5).toFixed(2), 1)) % -360; //.toFixed(2)
	}
	...
	checkCollision(otherBeyblade, ctx) {
		...
		if (distance < this.radius + otherBeyblade.radius && this.rpm > 0 && otherBeyblade.rpm > 0) {
			...
			const thisForceX =
				((xMagnitude * (otherBeyblade.rpm / 3000) * (otherWeightFactor / thisWeightFactor) * otherBeyblade.typeAttackFactor) / this.typeDefenseFactor) *
				randomNumberBetween(0.5, 1.5);
			const thisForceY =
				((yMagnitude * (otherBeyblade.rpm / 3000) * (otherWeightFactor / thisWeightFactor) * otherBeyblade.typeAttackFactor) / this.typeDefenseFactor) *
				randomNumberBetween(0.5, 1.5);

			const otherForceX =
				((xMagnitude * (this.rpm / 3000) * (thisWeightFactor / otherWeightFactor) * this.typeAttackFactor) / otherBeyblade.typeDefenseFactor) *
				randomNumberBetween(0.5, 1.5);
			const otherForceY =
				((yMagnitude * (this.rpm / 3000) * (thisWeightFactor / otherWeightFactor) * this.typeAttackFactor) / otherBeyblade.typeDefenseFactor) *
				randomNumberBetween(0.5, 1.5);
	...
	checkBoundaryCollision() {
		...
		if (
			...
			(this.x - this.radius < -maxBoundaryDistance && this.y > canvas.height / 2) ||
			(this.x + this.radius > canvasWidth + maxBoundaryDistance && this.y < canvas.height / 2)
		) {
		...
		if ((this.y - this.radius < 0 && this.x > canvas.width / 2) || (this.y + this.radius > canvasHeight && this.x < canvas.width / 2)) {
			...
			(this.y - this.radius < -maxBoundaryDistance && this.x < canvas.width / 2) ||
			(this.y + this.radius > canvasHeight + maxBoundaryDistance && this.x > canvas.width / 2)
		) {
			if (this.rpm > 0) this.rpm = 0;
			this.imageInUse = "static";
			this.image.src = this.slowImageSrc;
